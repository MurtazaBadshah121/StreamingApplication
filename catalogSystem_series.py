...
def SerieEntry():
    ...
    try:
        ...
        cursor.execute("""INSERT INTO Series (Title, Artist_Name, ReleasedDate, NumSeason, NumEpisode, Genre_Type) VALUES (?,?,?,?,?,?)""",
                   (sTitle_entry, nArtistName_entry, sDate_entry, nSeasons_entry, nEpisodes_entry, nGenre_entry))


        # commit the changes to db
        conn.commit()

        tk.Label(windowCat, text="Serie entry has been recorded.").grid(row=8)
...
def sCatalog():
    ...
    tk.Button(windowCat, text = "Back",height="2", width="30", command=windowCat.destroy).grid(row=6,pady=(20,10), padx=(150,10))
    tk.Button(windowCat, text = "Save",height="2", width="30", command=SerieEntry).grid(row=7,pady=(20,10), padx=(150,10))
