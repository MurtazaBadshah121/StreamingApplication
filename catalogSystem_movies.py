...
def movieEntry():
    ...
    try:
        ...
        cursor.execute("""INSERT INTO Movies (Title, Artist_Name, ReleasedDate, Duration, Genre_Type, MovieLikes) VALUES (?,?,?,?,?,?)""",
                   (mtitle_entry, artistname_entry, mdate_entry, mduration, genre_entry, mLikes_entry))
...
def mCatalog():
    ...
    tk.Label(windowCat, text="MovieLikes: ").grid(row=5,pady=(20,10), padx=(10,10))
    ...
    tk.Button(windowCat, text = "Back",height="2", width="30", command=exitmovieCat).grid(row=6,pady=(20,10), padx=(200,10))
    tk.Button(windowCat, text = "Save",height="2", width="30", command=movieEntry).grid(row=7,pady=(20,10), padx=(200,10))
    ...
    mLikes.grid(row=5, column=1, pady=(20,10), padx=(20,20))
